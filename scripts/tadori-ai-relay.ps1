...
Write-Host "  POST $baseUrlShort/tadori/pdf-extract      (PDF を PdfPig でテキスト抽出: body=octet-stream, header X-Tadori-Filename)"
...
# ─── PDF テキスト抽出 (PdfPig / Office 不要) ────────────────────────────────
# scripts/lib/pdfpig/ の DLL (Apache-2.0) を遅延ロードして PDF からテキストを抽出。
# Word は使わない。PowerShell 5.1 / .NET Framework 4.7.1+ で動作。

$script:PdfPigLoaded = $false
function Ensure-PdfPig {
    if ($script:PdfPigLoaded) { return $true }
    $libDir = Join-Path $PSScriptRoot 'lib\pdfpig'
    $main = Join-Path $libDir 'UglyToad.PdfPig.dll'
    if (-not (Test-Path -LiteralPath $main)) {
        Write-Host "[pdf] !! PdfPig DLL が見つかりません: $main" -ForegroundColor Red
        return $false
    }
    try {
        ...
        $script:PdfPigLoaded = $true
        Write-Host "[pdf] PdfPig ロード完了 ($libDir)" -ForegroundColor Green
        return $true
    } catch {
        Write-Host ("[pdf] !! PdfPig ロード失敗: {0}" -f $_.Exception.Message) -ForegroundColor Red
        return $false
    }
}

# POST /tadori/pdf-extract
# 入力: 生 PDF バイナリ (octet-stream) + Header X-Tadori-Filename
# 出力: { ok, text }
function Invoke-PdfExtract {
    ...
    if ($bytes.Length -lt 10) { Send-Error -Response $response -Status 400 -Code 'empty_body' -Detail 'PDF が空です'; return }

    $origName = ''
    try { $origName = [System.Uri]::UnescapeDataString([string]$request.Headers['X-Tadori-Filename']) } catch { }
    if (-not $origName) { $origName = 'input.pdf' }
    Write-Host ("[pdf] >>> extract 開始: {0} ({1} KB)" -f $origName, [int]($bytes.Length/1024)) -ForegroundColor Cyan

    if (-not (Ensure-PdfPig)) {
        Send-Error -Response $response -Status 503 -Code 'no_pdfpig' -Detail 'PdfPig DLL をロードできませんでした (scripts/lib/pdfpig/ を確認)'; return
    }

    $pdf = $null
    try {
        $pdf = [UglyToad.PdfPig.PdfDocument]::Open($bytes)
        $sb = New-Object System.Text.StringBuilder
        $n = $pdf.NumberOfPages
        for ($i = 1; $i -le $n; $i++) {
            $page = $pdf.GetPage($i)
            [void]$sb.AppendLine([string]$page.Text)
        }
        $text = $sb.ToString()
        Write-Host ("[pdf] <<< 完了: {0} ({1} ページ / {2} chars)" -f $origName, $n, $text.Length) -ForegroundColor Green
        Send-Json -Response $response -Status 200 -Body @{ ok = $true; text = $text }
    } catch {
        Write-Host ("[pdf] !! ERROR: {0}" -f $_.Exception.Message) -ForegroundColor Red
        Send-Error -Response $response -Status 500 -Code 'pdf_error' -Detail $_.Exception.Message
    } finally {
        if ($pdf) { try { $pdf.Dispose() } catch { } }
    }
}
...
function Invoke-RelayRequest {
    ...
    # ── ローカル機能: PDF テキスト抽出 (PdfPig / Office 不要) ──
    if ($path -eq '/tadori/pdf-extract')  { Invoke-PdfExtract  -Context $Context; return }
