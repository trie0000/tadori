...
- 引用カードに:
  ...
  - サムネイル (スライド PNG)
  - 「PowerPoint で開く」ボタン (relay 経由で該当スライドへジャンプ)
...
| # | 項目 | 候補 |
|---|---|---|
| 1 | PowerPoint プロセスを毎回 Quit() するか、relay 起動中ずっと保持するか | 毎回 Quit (安全) / プロセス維持 (高速だが管理難) |
| 2 | スピーカーノートは取り込むか | スライド本文に concat / 別 chunk / 取り込まない (本人合意で) |
| 3 | チャンク粒度 | 1 スライド = 1 chunk / 複数スライドをセクション単位で連結 |
| 4 | フォルダ URL 入力の形式 | 絶対 URL / serverRelativeUrl / 両対応 |
| 5 | サムネサイズ | 200x150 (引用カード) / フル解像度も保持 / 引用時のみフルロード |
| 6 | 同期トリガ | 手動のみ / writer 担当者が定期自動同期 / Sticky モードに統合 |
| 7 | 取り込み中の UI | プログレスバー / トーストの連発 / モーダルでブロック |
