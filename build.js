...
// pdf.js worker のソースを文字列モジュールとして生成 (Blob URL でワーカ起動するため)。
// node_modules の pdf.worker.min.mjs を src/docs/pdfWorker.generated.ts に焼き込む。
// 生成物は .gitignore 済み。
{
  const workerSrc = 'node_modules/pdfjs-dist/build/pdf.worker.min.mjs';
  const outTs = 'src/docs/pdfWorker.generated.ts';
  try {
    const src = fs.readFileSync(workerSrc, 'utf8');
    const ts = `// AUTO-GENERATED by build.js — do not edit. (pdf.js worker source)\nexport default ${JSON.stringify(src)};\n`;
    // 既存と同じなら書き換えない (watch の無限ループ防止)
    let prev = '';
    try { prev = fs.readFileSync(outTs, 'utf8'); } catch { /* none */ }
    if (prev !== ts) { fs.writeFileSync(outTs, ts); console.log(`[build] pdf worker 生成: ${outTs} (${(src.length / 1024).toFixed(0)} KB)`); }
  } catch (e) {
    console.warn(`[build] pdf worker 生成スキップ (${workerSrc} が無い?): ${e.message}`);
    // フォールバック空モジュール (PDF 機能は無効になるが他は動く)
    if (!fs.existsSync(outTs)) fs.writeFileSync(outTs, 'export default "";\n');
  }
}
